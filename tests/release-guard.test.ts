...
function fixture() {
  ...
  assets.push(...assets.filter(asset=>asset.file.startsWith("dist/")).map(asset=>({...asset,file:asset.file.replace(/^dist\//,"demo-dist/sdk/")})));
  ...
  const report:any={schemaVersion:1,version:"0.1.0",status:"passed",testedAt:"2026-09-17T00:00:00Z",sourceCommit:"a".repeat(40),distributionVerifiedAt:"2026-09-17T00:00:00Z",origin,servedAssets,model,assets,results:model.sources.flatMap(source=>["wasm","webgpu"].flatMap(backend=>["main","worker"].map(executionMode=>({source:source.kind,backend,executionMode,status:"passed",keypoints:17,actualBackend:backend,modelSha256:model.sha256,revision:source.revision}))))};
