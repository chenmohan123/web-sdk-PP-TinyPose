...
0.3.0 在2026-09-18的桌面验收将三模型 × WASM/WebGPU × main/Worker 12组合各运行至少30个变化帧，记录模型/实际后端、有限17点、会话加载次数、最大并发、暂停与停止后的回写；[报告](../../reports/2026-09-17-media/media-acceptance.json)含实际浏览器、OS和构建摘要。12组合使用同一 Demo 控制器和正式 SDK 资产的独立验收页；正式生产 Demo UI 另验默认配置的本地视频及 Chromium fake-device 摄像头。测试素材是单张人物图的平移缩放，不是动作数据集；fake-device、390px均不是物理摄像头或手机验证。
