...
0.2.0 候选在同一桌面环境进一步覆盖 128×96 FP32、旧 256×192 FP32 与 128×96 W16A32 的 WASM/WebGPU × main/Worker，共 12 项均通过；见 [浏览器对比](../../reports/2026-09-17-variants/browser-comparison.json)。W16A32 跨后端最大可靠点差为 `0.001567px`。三模型双源文件与卡已完成固定 revision 完整 GET，见 [变体分发回执](../../reports/2026-09-17-variants/distribution-variants-verified.json)。候选发布仍以当前构建的 24 项真实分发验收为准。
