...
- [ ] 核对官方增强版 128 ZIP 来源、资产与旧 256 基线；固定 SHA 与工具版本。
- [ ] 实现可复现转换，两个 FP16 保留 float32 I/O；先验证度量行为，再运行真实转换。
- [ ] 固定32框一致性与64图关键点 GT 子集，记录全量病例与预设门槛结果。
- [ ] 混合FP16失败后做一次两个规格W16A32替代实验，使用同一锁定样本与门槛，明确FP16存储/FP32计算，不覆盖失败证据。
- [ ] 自查、相关测试与提交；独立审查转换和质量结论。
...
- [ ] 先写支持128、非法尺寸、旧默认及Worker传递的关键行为测试并确认失败。
- [ ] 实现端到端规格传递、冻结副本、形状校验与原图坐标；保留生命周期语义。
- [ ] 运行单测、类型、构建；执行4模型×4运行组合的真实张量/公开SDK输入评测，复用固定样本与门槛，记录未支持组合。
- [ ] 自查、提交、独立审查。
...
- [ ] 准备达标模型双语卡、许可、转换与评测摘要，上传两源，完整GET核对并写入不可变revision。
- [ ] Demo提供规格与精度选择，模型切换取消旧操作/释放会话/更新缓存身份，保留可选框选和稳定布局，双语/390px验证。
- [ ] 更新来源回归、输入竞争、多模型构建资产/发布guard；运行每个发布模型×双源×CPU/GPU×main/worker验收。
- [ ] 更新文档比较表和版本0.2.0；全套单测/类型/构建/打包/标准检查，独立审查。

## Task 4：合并、发布及门户同步

- [ ] 全分支独立审查，记录并解决影响发布的问题。
- [ ] 按既有授权PR、CI、合并，使用release.yml发布真实0.2.0；核对npm包/provenance、Release、Pages真实资产和功能。
