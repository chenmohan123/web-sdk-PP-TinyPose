...
Version 0.3.0 adds no camera-permission or video-playback API. The Demo's `createMediaController` is not an npm public interface: it supplies sampled frames to the existing `run`, reuses one `load`, and permits at most one inference in flight. Render each result with its corresponding independently owned RGBA frame. Pause retains the session but discards late results; stop releases media tracks and calls `dispose` before awaiting completion. AbortSignal alone does not guarantee immediate Worker termination; `dispose` terminates the Worker, while submitted main-thread kernels may need to finish.
