...
- 2026-09-17 桌面 Chromium 153 / Windows 11 / ORT Web 1.27.0 的三模型 WASM/WebGPU × main/Worker 12 项候选比较通过；模型双源固定 revision 完整 GET 通过。正式发布还要求当前构建的 24 组合验收。
- npm 与生产 Demo 不含 ONNX；手机、微信 web-view、WebNN/NPU、自动多人检测和全量 COCO AP 未验证。

发布状态：模型已分发并回读；npm 0.2.0、GitHub Release 与 Pages 仍须以 Task 4 的真实服务回执为准。本条目不声明它们已发布。
